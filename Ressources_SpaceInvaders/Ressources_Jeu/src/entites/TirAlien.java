...
/**** METHODES ****/
	...
	public int[] tirAlienToucheChateau() { // Renvoie num�ro ch�teau touch� et abscisse du tir
		int[] tabRep = {-1,-1}; 
		if(this.tirAlienAHauteurDeChateau() == true) { // Le tir alien est � hauteur du ch�teau		
	...
	public void TirAlienDetruitChateau(Chateau tabChateaux[]) {
		int[] tab = this.tirAlienToucheChateau(); // Contient (-1,-1) ou le num�ro du ch�teau touch� et l'abscisse du tir
		if(tab[0] != -1) { // Un ch�teau est touch�
			if(tabChateaux[tab[0]].trouveBriqueHaut(tabChateaux[tab[0]].trouveColonneChateau(tab[1])) != -1
				...
				tabChateaux[tab[0]].casseBriquesHaut(tab[1]); // D�truit les briques du ch�teau touch�									
